    ...
    if total is not 0:
        for k in range(0, int(total/1000)+1):
            ...
            for i in result['hits']['hits']:
                ...
                title = None
                content = None
                if 'title' not in i['highlight'].keys():
                    title = i['_source']['title']
                else:
                    title = i['highlight']['title'][0]
                if 'content' not in i['highlight'].keys():
                    content = i['_source']['content']
                else:
                    content = i['highlight']['content'][0]
            ...
            if key is 'date':
                if value.has_key('type'):
                    if value['type'] is 'range':
                        ...
                        l = False
                        if value.has_key('week'):
                            data['range']['date']['lte'] = 'now'
                            data['range']['date']['gte'] = 'now-'+value['week']+'d/d'
                            l = True
                        if value.has_key('month'):
                            data['range']['date']['lte'] = 'now'
                            data['range']['date']['gte'] = 'now-'+value['week']+'d/d'
                            l = True
                        if value.has_key('year'):
                            data['range']['date']['lte'] = 'now'
                            data['range']['date']['gte'] = 'now-'+value['week']+'d/d'
                            l = True
                        k = False
                        ...
                        if not k or not l:
                            raise DSLError('range模式需要设置date字典中的lt、lte、gt、gte字段的数据！'
                                           '或者是date字典中缺失week、month、year字段数据！')
                    else:
                        data = {
                            'query_string': {
                                'date': {
